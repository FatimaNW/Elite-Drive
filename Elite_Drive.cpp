...
class Driver{
	...
	public:
		bool driver_register(){
			...
			cin >> phone_number;
			while (phone_number.length() != 11 || !isNumber(phone_number)) {
			...
			if (age >= 18) {
				cout << "Enter NIC: " ;
				cin >> NIC;
				while (NIC.length() != 13 || !isNumber(NIC)) {
...
public:	
	...
	void cancel_ride(){
		cout<<"\nEnter the ride ID you want to cancel: ";
		cin>>ID;
		list.delete_ride(ID);
	}
